...
void SDRecordModule::drawFrame(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x, int16_t y)
{
    ...
    graphics::drawCommonHeader(display, x, y, "GPS Record");
    ...
    if (m_fp_open) {
        display->drawString(x, y + (FONT_HEIGHT_SMALL * 1), String("Rec "));
        ...
        if (!m_wantRecord) {
            display->drawString(x, y + (FONT_HEIGHT_SMALL * 1), String("SD Off "));
            header_offset = display->getStringWidth("SD Off ");

        } else {
            display->drawString(x, y + (FONT_HEIGHT_SMALL * 1), String("SD Idle "));
            header_offset = display->getStringWidth("SD Idle ");
        }
    }
    if (m_noCard) {
        display->drawString(x + header_offset, y + (FONT_HEIGHT_SMALL * 1), String("No Card"));
    } else if (m_cantWrite) {
        display->drawString(x + header_offset, y + (FONT_HEIGHT_SMALL * 1), String("Error"));
    }
